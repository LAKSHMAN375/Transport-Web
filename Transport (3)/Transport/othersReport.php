...
$query = mysqli_query($db,"INSERT INTO `othersreport`  select m.busid,e.category,e.description,e.cost from `masterdemo` m left OUTER JOIN `others` e on m.busid=e.busid and e.fromdate BETWEEN '$fromdate' and '$todate'; ");  
...
$conn=new mysqli("localhost","root","","transport");
  if($conn->connect_error){
    ...
    $query=mysqli_query($conn,"select m.busid,e.category,e.description,e.cost from `masterdemo` m left OUTER JOIN `others` e on m.busid=e.busid and e.fromdate BETWEEN '$fromdate' and '$todate' ;");
    echo "<table border='2' style='border: 2px solid black; color:black; background-color:white;'>
    <tr>
    <th colspan=5>Other Repairs</th></tr>
    <tr>
    <th>BUS_ID</th>
    <th>CATEGORY</th> 
    <th>Description</th>
